...
DEFAULT_CORE="$(cd "$ROOT/.." 2>/dev/null && pwd)/ywd-hotspot"
DIST="$ROOT/dist"
...
core_path() {
  local configured
  configured="$(cfg_get core_path "$DEFAULT_CORE")"
  printf '%s\n' "$configured"
}
...
meta() {
  local id="$1"
  python3 - "$ROOT/plugins/$id/plugin.json" <<'PY'
import json, pathlib, sys
p=pathlib.Path(sys.argv[1]); d=json.loads(p.read_text())
print(str(d.get('id') or ''))
print(str(d.get('version') or ''))
print(str(d.get('kind') or ''))
print(str(d.get('name') or ''))
PY
}

selected_plugin() {
  local id="${1:-$(cfg_get selected_plugin '')}"
  if [[ -n "$id" && -f "$ROOT/plugins/$id/plugin.json" ]]; then printf '%s\n' "$id"; return; fi
  id="$(plugin_ids | head -n1 || true)"
  [[ -n "$id" ]] || die "No plugin source directories were found under plugins/"
  cfg_set selected_plugin "$id"
  printf '%s\n' "$id"
}

select_plugin_interactive() {
  mapfile -t ids < <(plugin_ids)
  ((${#ids[@]})) || die "No plugins found"
  printf '\nAvailable plugins:\n'
  local i
  for i in "${!ids[@]}"; do printf ' %2d) %s\n' "$((i+1))" "${ids[$i]}"; done
  read -r -p "Select plugin: " i
  [[ "$i" =~ ^[0-9]+$ ]] || die "Invalid selection"
  ((i>=1 && i<=${#ids[@]})) || die "Invalid selection"
  cfg_set selected_plugin "${ids[$((i-1))]}"
  printf 'Selected: %s\n' "${ids[$((i-1))]}"
}

validate_plugin() {
  local id core src
  id="$(selected_plugin "${1:-}")"; core="$(ensure_core)"; src="$ROOT/plugins/$id"
  PYTHONPATH="$core/lib" python3 - "$src/plugin.json" <<'PY'
import json, pathlib, sys
path=pathlib.Path(sys.argv[1]); raw=json.loads(path.read_text()); kind=str(raw.get('kind') or '')
if kind=='declarative':
    import plugin_manager as m
elif kind=='service':
    import plugin_service_manager as m
elif kind=='ui':
    import plugin_ui_manager as m
else:
    raise SystemExit(f'unsupported plugin kind: {kind or "?"}')
out=m.validate_manifest(path)
...
key_paths_ok() {
  local core private public root_real core_real private_real public_real
  core="$(ensure_core)"
  root_real="$(realpath -m "$ROOT")"; core_real="$(realpath -m "$core")"
...
setup_keys() {
  need openssl; need realpath
  local key_id publisher key_dir private public existing
  key_id="$(cfg_get key_id 'kj6ywd-official-1')"
  publisher="$(cfg_get publisher 'KJ6YWD')"
  read -r -p "Key ID [$key_id]: " existing; key_id="${existing:-$key_id}"
  [[ "$key_id" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]{0,63}$ ]] || die "Invalid key ID"
  read -r -p "Publisher [$publisher]: " existing; publisher="${existing:-$publisher}"
  ...
  if [[ -e "$private" || -e "$public" ]]; then
    read -r -p "Key files already exist. Replace them? [y/N]: " existing
    [[ "$existing" =~ ^[Yy]$ ]] || { printf 'Canceled.\n'; return; }
  fi
...
output_for() {
  local id="$1" lines version
  mapfile -t lines < <(meta "$id"); version="${lines[1]}"
  printf '%s/%s-%s.ywdplugin\n' "$DIST" "$id" "$version"
}

build_plugin() {
  local id="$(selected_plugin "${1:-}")"" mode="${2:-unsigned}" core src out publisher key_id private
  core="$(ensure_core)"; src="$ROOT/plugins/$id"; out="$(output_for "$id")"
  mapfile -t info < <(meta "$id")
  local kind="${info[2]}"
  validate_plugin "$id"
  mkdir -p "$DIST"
  publisher="$(cfg_get publisher 'KJ6YWD')"
  if [[ "$mode" == "unsigned" ]]; then
    ...
    key_id="$(cfg_get key_id '')"; private="$(cfg_get private_key '')"
    [[ -n "$key_id" && -f "$private" ]] || die "Signing key is not configured. Run Signing Key Setup first."
    key_paths_ok "$private" "$(cfg_get public_key '')"
...
inspect_package() {
  local package="${1:-}"
  if [[ -z "$package" ]]; then package="$(output_for "$(selected_plugin)")"; fi
...
trust_command() {
  local key_id public
  key_id="$(cfg_get key_id '')"; public="$(cfg_get public_key '')"
  ...
  printf 'Local public key: %s\nPrivate key stays here: %s\n' "$public" "$(cfg_get private_key '')"
}

clean_dist() {
  mkdir -p "$DIST"
  find "$DIST" -maxdepth 1 -type f -name '*.ywdplugin' -delete
  printf 'Cleaned %s\n' "$DIST"
}

status_block() {
  local core id info branch version
  core="$(ensure_core)"; id="$(selected_plugin)"; mapfile -t info < <(meta "$id")
  branch="$(git -C "$core" branch --show-current 2>/dev/null || printf '?')"
  version="$(cat "$core/VERSION" 2>/dev/null || printf '?')"
  printf ' Core:        %s\n' "$core"
  printf ' Core branch: %s\n' "$branch"
  printf ' Core ver:    %s\n' "$version"
  printf ' Plugin:      %s v%s (%s)\n' "$id" "${info[1]}" "${info[2]}"
  printf ' Key ID:      %s\n' "$(cfg_get key_id 'not configured')"
}
...
EOF
    read -r -p '> ' choice
    case "$choice" in
      1) select_plugin_interactive;;
      2) validate_plugin;;
      3) build_plugin "" unsigned;;
      4) build_plugin "" signed;;
      5) inspect_package;;
      6) verify_package;;
      7) setup_keys;;
      8) trust_command;;
...
case "$cmd" in
  menu) menu;;
  list) plugin_ids;;
  select) [[ -n "${2:-}" ]] || die "usage: $0 select PLUGIN_ID"; selected_plugin "$2" >/dev/null; cfg_set selected_plugin "$2"; printf 'Selected: %s\n' "$2";;
