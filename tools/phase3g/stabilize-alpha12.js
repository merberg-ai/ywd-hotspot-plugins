'use strict';
(() => {
  // Phase 3G Alpha12 stabilization layer.
  //
  // Alpha11 proved the real recovered-AMBE49 -> external vocoder -> browser PCM
  // path, but an external RESET is much more expensive than the old in-browser
  // decoder reset.  Do not turn ordinary sequence discontinuities into remote
  // RESET traffic. True call/gap boundaries are still detected independently by
  // the Alpha11 engine and continue to reset the backend.
  ...
  function init() {
    ...
    const root = byId('ywd-plugin-root');
    if (root) {
      new MutationObserver(renderStats).observe(root, {childList:true, subtree:true, characterData:true, attributes:true});
    }
  }
